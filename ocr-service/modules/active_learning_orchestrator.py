...
from modules.active_learning import HybridSampling, OCRModel
from modules.learning_engine import LearningEngine
from utils.validation import validate_ocr_batch
from utils.drift_detection import check_for_drift
...
class ALOrchestrator:
    ...
    async def run_cycle(self, n_samples: int = 50) -> Dict[str, Any]:
        ...
        if not recent_data or len(recent_data) < n_samples:
            logger.warning("Insufficient data to run AL cycle. Found: %d", len(recent_data))
        ...
        logger.info("Selected %d candidates for labeling via Hybrid Strategy.", len(candidates))
        ...
        return {
            ...
            "candidates": candidates.to_dict(orient="records")[:5] # Return top 5 for preview
        }
    ...
    async def _fetch_recent_results(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        def _fetch():
            return self.learning_engine.client.table("learning_patterns")\
                .select("*")\
    ...
    def _prepare_for_validation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Supabase schema: [id, doc_type, font_metadata, accuracy_score, created_at, version]
        v_df = pd.DataFrame()
        v_df["image_path"] = df.get("id", "unknown").apply(lambda x: f"s3://bucket/{x}.png")
