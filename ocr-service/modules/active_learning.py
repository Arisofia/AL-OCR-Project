...
class QueryStrategy:
    ...
    def select_indices(self, model: OCRModel, unlabeled_data: np.ndarray, n_samples: int) -> List[int]:
    ...
    def select_indices(self, model: OCRModel, unlabeled_data: np.ndarray, n_samples: int) -> List[int]:
        ...
        if len(selected_indices) > n_samples:
            ...
            if len(remaining_indices) > 0:
                ...
                extra_local = np.argsort(uncertainty[remaining_indices])[::-1][:extra_needed]
