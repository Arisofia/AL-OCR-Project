...
def test_pipeline_on_pixelated_sample(tmp_path):
    ...
    assert os.path.exists(img_path), "Generate sample images by running tests/generate_samples.py"

    worker = IterativeOCR(img_path, iterations=2, save_iterations=True, output_dir=str(tmp_path))
