...
- `LAMBDA_FUNCTION_NAME` may be a plain name or full Lambda ARN; `deploy.sh`
  normalizes ARN input to a valid function name.
...
- OCR strategy profile: choose `OCR_STRATEGY_PROFILE` to tune behavior by workload:
